...
Nifly and DirectXTex may remain static libraries, but the release does not require a fully static native dependency closure. Select one CRT policy across Rust, the C++ bridges, and every native library. `/MT` statically links the multithreaded CRT while `/MD` uses its DLL form, and Microsoft requires all modules passed to one link to use the same runtime option ([MSVC runtime options](https://learn.microsoft.com/en-us/cpp/build/reference/md-mt-ld-use-run-time-library)). The selected ZIP policy is `/MD` plus Rust's default dynamic CRT, subject to a clean link and license review; no debug CRT may appear in release imports.
...
Run the same closure check on delay-load imports and helper executables. The supported Microsoft Visual C++ Redistributable is an explicit installation prerequisite for the selected `/MD` policy; record its supported version and acquisition instructions in release metadata and clean-machine test setup. Microsoft limits redistribution to listed files under the Visual Studio license, so copying DLLs from a build machine is forbidden ([Visual C++ redistribution](https://learn.microsoft.com/en-us/cpp/windows/redistributing-visual-cpp-files)).
...
7. **Clean-machine smoke:** install and verify the declared Microsoft Visual C++ Redistributable prerequisite, then extract with Windows' built-in ZIP support to paths containing spaces, non-ASCII, and near-long-path boundaries on clean Windows 10 22H2 and Windows 11 x64 VMs with no Rust, Visual Studio, or repository. Launch from Explorer and PowerShell, verify GUI/icon/version, create the portable state tree, run a representative operation through every backend/helper, cancel a run, relaunch, and uninstall by deleting the directory.
...
- Use `/MD` and Rust's default dynamic CRT consistently. The supported Microsoft Visual C++ Redistributable is an explicit prerequisite. The release owner must verify the pinned Visual Studio Build Tools/Windows SDK terms and the final recursive import closure; any other required non-system DLL becomes an application-local, inventoried component or blocks release.
...
Build a deterministic, single-root Windows x64 ZIP from a committed production lockfile and exact native pins; use one consistent dynamic CRT policy with a documented Visual C++ Redistributable prerequisite; embed complete Windows metadata; audit recursive PE dependencies; and prove the exact staged package on clean Windows 10/11 VMs. Ship notices, CycloneDX SBOM, hashes, release metadata, and GitHub attestations inside or beside the artifact, with symbols separate and the initial PE files explicitly unsigned.
