...
OperationResult::OperationResult(const bool succeeded,
                                 const bool wouldChange,
                                 std::string message)
    : _succeeded(succeeded)
    , _wouldChange(wouldChange)
    , _message(std::move(message))
{}

OperationResult OperationResult::changed() noexcept
{
    return OperationResult(true, true, {});
}

OperationResult OperationResult::unchanged() noexcept
{
...
bool OperationResult::wouldChange() const noexcept
{
    return _wouldChange;
}

const std::string &OperationResult::message() const noexcept
{
    return _message;
}

AssetExecutionResult::AssetExecutionResult(
    std::optional<AssetExecutionFailure> failure,
    std::string message)
    : _failure(failure)
    , _message(std::move(message))
{}
...
bool AssetExecutionResult::succeeded() const noexcept
{
...
const std::string &AssetExecutionResult::message() const noexcept
{
    return _message;
}

AssetExecutor::AssetExecutor(AssetExecutionBackend &backend) noexcept
    : _backend(backend)
{}
...
AssetExecutionResult AssetExecutor::executeTexture(const routing::RoutedAsset &asset) const
{
    ...
    const auto operation = _backend.optimizeTexture(asset.operations(),
                                                    asset.executionMode());
...
AssetExecutionResult AssetExecutor::executeMesh(const routing::RoutedAsset &asset) const
{
    ...
    if (asset.operations().contains(routing::AssetOperation::Optimization)) {
        const auto optimization = _backend.optimizeMesh(asset.executionPath(),
                                                        asset.executionMode());
        if (!optimization.succeeded())
            return operationFailure(optimization);
    ...
    if (asset.operations().contains(routing::AssetOperation::MeshReferenceMaintenance)) {
        ...
        if (!maintenance.succeeded())
            return operationFailure(maintenance);
...
AssetExecutionResult AssetExecutor::executeAnimation(const routing::RoutedAsset &asset) const
{
    ...
    const auto operation = _backend.optimizeAnimation(asset.executionPath(),
                                                      asset.executionMode());
    if (!operation.succeeded())
        return operationFailure(operation);
    return AssetExecutionResult::success();
