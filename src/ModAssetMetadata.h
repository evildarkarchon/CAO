...
public:
     ...
     * \param fileName The Profile file name to resolve, including fallback behavior.
     * \return Non-comment, non-empty entries from the resolved file, or an empty list when it is unavailable.
     */
    [[nodiscard]] virtual QStringList readReferenceList(const QString &fileName) const = 0;
};
...
public:
     ...
     */
    [[nodiscard]] virtual QStringList listHeadparts(const QString &pluginPath) const = 0;
};
...
public:
     ...
     * \param headpartMeshPaths Profile-provided and plugin-derived Headpart mesh paths.
     */
     ...
     * \return True when the path matches Profile/plugin Headpart metadata or the FaceGen convention.
     */
...
public:
     ...
     * \return Metadata derived only from the selected Mods and active Profile reference lists.
     */
    [[nodiscard]] virtual ModAssetMetadata buildForMods(const QStringList &selectedMods) const = 0;
};
...
public:
    /*!
     * \brief Creates a builder from the adapters needed to read Profile and plugin references.
     ...
     */
    ModAssetMetadataBuilder(const ProfileAssetReferenceProvider &profileReferences,
                            const PluginAssetReferenceReader &pluginReferences);

    /*!
     * \brief Builds Mod Asset Metadata for selected Mods.
     * \param selectedMods Mods selected by the Asset Work Plan.
     * \return Metadata combining Profile Headpart references and plugin-derived Headpart references.
     */
    [[nodiscard]] ModAssetMetadata buildForMods(const QStringList &selectedMods) const override;
...
class ProfileFileAssetReferenceProvider final : public ProfileAssetReferenceProvider
{
public:
    /*!
     * \brief Reads reference lists through the active Profile file lookup rules.
     * \param fileName The Profile file name to read.
     * \return Non-comment, non-empty entries from the current Profile or its fallback Profile.
     */
    [[nodiscard]] QStringList readReferenceList(const QString &fileName) const override;
};

class PluginOperationsAssetReferenceReader final : public PluginAssetReferenceReader
{
public:
    /*!
     ...
     * \return Headpart mesh paths referenced by the plugin.
     */
    [[nodiscard]] QStringList listHeadparts(const QString &pluginPath) const override;
};
