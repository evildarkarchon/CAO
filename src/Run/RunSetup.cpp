...
std::string malformedArchiveExtensionReason(const routing::MalformedArchiveExtensionReason reason) {
    switch (reason) {
...
routing::RoutingPolicyBuildResult RunSetup::prepare(const ApplicationRunChoices& choices,
        ...
        Overloaded{
            ...
            [](const routing::MalformedArchiveExtension& malformed) {
                return "The selected profile Archive extension '" + malformed.extension +
                       "' is invalid because " + malformedArchiveExtensionReason(malformed.reason) +
                       ".";
            },
            [](const routing::AmbiguousArchiveExtension& ambiguous) {
                return "The selected profile Archive extension '" + ambiguous.extension +
                       "' conflicts with the built-in " + assetKindName(ambiguous.conflictingKind) +
                       " extension '" + ambiguous.conflictingExtension + "'.";
            },
