...
/// Chooses one outcome from sealed work and cleanup facts after every producer has stopped.
RunOutcome classifyTerminalOutcome(const RunEvidence& evidence) {
    bool unsafe = !evidence.failures().empty();
    bool containedFailure = false;
    const auto account = [&](const execution::MutationState mutation, const bool succeeded,
                             const bool safeToContinue) {
        unsafe =
            unsafe || !safeToContinue || mutation == execution::MutationState::PartialOrUnknown;
        containedFailure = containedFailure || !succeeded;
    };
    for (const auto& attempt : evidence.assetAttempts())
        account(attempt.result.mutationState(), attempt.result.succeeded(),
                attempt.result.safeToContinue());
    for (const auto& attempt : evidence.archiveExtractionAttempts())
        account(attempt.mutation, attempt.succeeded(), attempt.safeToContinue);
    if (const auto* finalization = evidence.archiveFinalization()) {
    ...
    // Unsafe work wins over cancellation. Cancellation observed during cleanup still wins over
    // cleanup errors, while an unexpected cleanup service exception makes other runs fatal.
    if (unsafe) return RunOutcome::Failed;
    if (evidence.cancellationObserved()) return RunOutcome::Cancelled;
    const auto cleanupFailures = evidence.cleanupFailures();
    ...
    if (containedFailure || !cleanupFailures.empty()) return RunOutcome::CompletedWithFailures;
...
OptimizationRunResult RunExecutor::schedulingFailure(
    ...
    const auto outcome = classifyTerminalOutcome(terminalEvidence);
    return OptimizationRunResult::terminal(outcome, RunPhase::Preparing,
                                           std::move(terminalEvidence), std::move(runId));
}
