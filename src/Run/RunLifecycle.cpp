...
std::span<const RunPhase> runPhaseSequence() noexcept
{
    ...
    static constexpr std::array sequence{RunPhase::Preparing,
...
RunProgress::RunProgress(const std::size_t total,
...
RunProgress RunProgress::determinate(const std::size_t total,
                                     const std::size_t succeeded,
                                     const std::size_t failed) noexcept
{
...
std::size_t RunProgress::total() const noexcept
{
    return _total;
}

std::size_t RunProgress::succeeded() const noexcept
{
    return _succeeded;
}

std::size_t RunProgress::failed() const noexcept
{
    return _failed;
}
...
RunPhaseRecord::RunPhaseRecord(const RunPhase phase,
                               const RunPhaseStatus status,
...
RunPhaseRecord RunPhaseRecord::skipped(const RunPhase phase, const PhaseSkipReason reason) noexcept
{
...
RunPhase RunPhaseRecord::phase() const noexcept
{
    return _phase;
}

RunPhaseStatus RunPhaseRecord::status() const noexcept
{
    return _status;
}

std::optional<PhaseSkipReason> RunPhaseRecord::skipReason() const noexcept
{
    return _skipReason;
}

const std::optional<RunProgress> &RunPhaseRecord::progress() const noexcept
{
    return _progress;
}
...
ModSelectionKind ModSelection::kind() const noexcept
{
    return _kind;
}

const std::filesystem::path &ModSelection::directory() const noexcept
{
    return _directory;
}

RunRequest::RunRequest(std::string profileIdentity,
                       const routing::ExecutionMode executionMode,
                              ...
                              std::vector<routing::RequestedWork> requestedWork)
{
    ...
    requestedWork.erase(std::unique(requestedWork.begin(), requestedWork.end()),
                        requestedWork.end());
    return RunRequest(std::move(profileIdentity),
                      executionMode,
                      std::move(modSelection),
                      std::move(requestedWork));
}

const std::string &RunRequest::profileIdentity() const noexcept
{
    return _profileIdentity;
}

routing::ExecutionMode RunRequest::executionMode() const noexcept
{
    return _executionMode;
}

const ModSelection &RunRequest::modSelection() const noexcept
{
    return _modSelection;
}
...
bool RunRequest::hasRequestedWork() const noexcept
{
    return !_requestedWork.empty();
}

OptimizationRunResult::OptimizationRunResult(const RunOutcome outcome,
...
RunOutcome OptimizationRunResult::outcome() const noexcept
{
    return _outcome;
}

RunPhase OptimizationRunResult::finalPhase() const noexcept
{
    return _finalPhase;
}
...
{
    const auto record = std::find_if(_phases.begin(), _phases.end(), [phase](const auto &candidate) {
