...
public:
  static util::nstring header() {
    // For spacing
    return util::nstring(
        L"<style>html{line-height:1.5rem}pre{line-height:1rem}</style>");
  }
  ...
  static util::nstring format(const Record &record) {
    ...
    ss << PLOG_NSTR("<br>")
       ...
       << PLOG_NSTR("} ") << message << PLOG_NSTR("</font>");
...
public:
  ...
  static util::nstring format(const Record &record) {
    ...
    ss << PLOG_NSTR("<br>")
       ...
       << std::left
       // Actual message
       << PLOG_NSTR(" [") << severityToString(record.getSeverity())
...
inline void initCustomLogger(const QString &logPath, bool debugLog) {
  ...
  static plog::RollingFileAppender<plog::CustomDebugFormatter> debugAppender(
      qPrintable(logPath), 250'000, 1'000);

  static plog::RollingFileAppender<plog::CustomInfoFormatter> infoAppender(
      qPrintable(logPath), 250'000, 1'000);
