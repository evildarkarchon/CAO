...
constexpr std::array requestedWorkDefinitions{
    RequestedWorkDefinition{RequestedWork::NativeTextureOptimization,
                            ProfileCapability::NativeTextureOptimization, TextureVariant::Native},
    ...
    RequestedWorkDefinition{RequestedWork::StandardMeshOptimization,
                            ProfileCapability::StandardMeshOptimization, MeshVariant::Standard},
    RequestedWorkDefinition{RequestedWork::TerrainMeshOptimization,
                            ProfileCapability::TerrainMeshOptimization, MeshVariant::Terrain},
...
constexpr std::array profileCapabilityDefinitions{
    ProfileCapabilityDefinition{ProfileCapability::NativeTextureOptimization, AssetKind::Texture},
    ProfileCapabilityDefinition{ProfileCapability::ConvertibleTextureConversion,
                                AssetKind::Texture},
    ProfileCapabilityDefinition{ProfileCapability::StandardMeshOptimization, AssetKind::Mesh},
...
constexpr std::array builtInExtensions{
    BuiltInExtension{".dds", AssetKind::Texture}, BuiltInExtension{".tga", AssetKind::Texture},
    BuiltInExtension{".nif", AssetKind::Mesh},    BuiltInExtension{".btr", AssetKind::Mesh},
    BuiltInExtension{".bto", AssetKind::Mesh},    BuiltInExtension{".hkx", AssetKind::Animation}};

/// Returns the terminal extension using the ASCII case rules that define supported Asset
/// extensions.
...
void include(std::array<bool, Size>& values, const Enum value) noexcept {
    const auto index = static_cast<std::size_t>(value);
    if (index < values.size()) values[index] = true;
}
...
AssetKind assetKindFor(const ProfileCapability capability) noexcept {
    for (const auto& definition : profileCapabilityDefinitions) {
        if (definition.capability == capability) return definition.kind;
    }
...
std::optional<cao::routing::MalformedArchiveExtensionReason> malformedReason(
    ...
    if (extension.front() != '.') return MalformedArchiveExtensionReason::MissingLeadingPeriod;
    if (extension.size() == 1) return MalformedArchiveExtensionReason::EmptySuffix;

    for (const auto character : extension.substr(1)) {
        const bool isAsciiLetter =
            (character >= 'A' && character <= 'Z') || (character >= 'a' && character <= 'z');
        const bool isDigit = character >= '0' && character <= '9';
        if (!isAsciiLetter && !isDigit) return MalformedArchiveExtensionReason::InvalidCharacter;
    }

    return std::nullopt;
}

/// Selects const Routed Asset references by one carried execution fact while preserving ledger
/// order.
...
RoutedAssetReferences matchingRoutedAssets(const std::span<const RoutedAsset> routedAssets,
                                           Projection projection, const Value expected) {
...
RoutingPolicyRequest RoutingPolicyRequest::forWork(
    const ExecutionMode mode, const std::initializer_list<RequestedWork> work) noexcept {
    return fromWork(mode, work);
}

RoutingPolicyRequest RoutingPolicyRequest::forWork(
    const ExecutionMode mode, const std::vector<RequestedWork>& work) noexcept {
    return fromWork(mode, work);
}

RoutingPolicyRequest RoutingPolicyRequest::fromWork(
    const ExecutionMode mode, const std::span<const RequestedWork> work) noexcept {
    auto request = RoutingPolicyRequest();
    request._executionMode = mode;
    for (const auto choice : work) include(request._work, choice);
...
ProfileCapabilities ProfileCapabilities::define(
    std::string archiveExtension, const std::initializer_list<ProfileCapability> capabilities) {
...
ProfileCapabilities ProfileCapabilities::fromDefinition(
    ...
    for (const auto capability : capabilities) include(profile._capabilities, capability);

    return profile;
}

RoutingPolicyBuildResult RoutingPolicy::compile(RoutingPolicyRequest request,
    ...
    if (!capabilities._archiveExtension.has_value() || capabilities._archiveExtension->empty()) {
        ...
        for (const auto& builtIn : builtInExtensions) {
            if (normalizedArchiveExtension == builtIn.extension) {
                errors.emplace_back(AmbiguousArchiveExtension{
                    *capabilities._archiveExtension, std::string(builtIn.extension), builtIn.kind});
                break;
            }
        }
    }

    // Validate every explicit choice before derived work so adapters receive the complete conflict
    // set in one pass.
    for (const auto& definition : requestedWorkDefinitions) {
        if (!contains(request._work, definition.work)) continue;

        if (contains(capabilities._capabilities, definition.requiredCapability)) continue;
        ...
        if (definition.variant.has_value()) {
            errors.emplace_back(
                UnsupportedRequestedAssetVariant{definition.work, *definition.variant});
        } else {
            errors.emplace_back(UnsupportedRequestedAssetKind{definition.work, kind});
        }
    }

    const bool maintainsMeshReferences =
        contains(request._work, RequestedWork::ConvertibleTextureConversion);
    if (maintainsMeshReferences &&
        !contains(capabilities._capabilities, ProfileCapability::MeshReferenceMaintenance)) {
        errors.emplace_back(UnsupportedDerivedOperation{RequestedWork::ConvertibleTextureConversion,
                                                        AssetOperation::MeshReferenceMaintenance});
    }

    if (!errors.empty()) return RoutingPolicyBuildResult(std::move(errors));

    return RoutingPolicyBuildResult(RoutingPolicy(request._executionMode, request._work,
...
ExecutionMode RoutingPolicy::executionMode() const noexcept { return _executionMode; }

bool RoutingPolicy::requests(const RequestedWork work) const noexcept {
    return contains(_work, work);
}

bool RoutingPolicy::maintainsMeshReferences() const noexcept { return _meshReferenceMaintenance; }

const std::string& RoutingPolicy::archiveExtension() const noexcept { return _archiveExtension; }
...
TextureAsset::TextureAsset(const TextureVariant variant) noexcept : _variant(variant) {}

TextureVariant TextureAsset::variant() const noexcept { return _variant; }

MeshAsset::MeshAsset(const MeshVariant variant) noexcept : _variant(variant) {}

MeshVariant MeshAsset::variant() const noexcept { return _variant; }
...
bool AssetOperations::empty() const noexcept {
    return std::none_of(_operations.begin(), _operations.end(),
                        [](const bool included) { return included; });
}

RoutedAsset::RoutedAsset(std::filesystem::path executionPath, AssetIdentity identity,
                         const RoutedAssetPhase phase, const OptimizerTarget target,
                         const ExecutionMode executionMode, AssetOperations operations)
...
const std::filesystem::path& RoutedAsset::executionPath() const noexcept { return _executionPath; }

AssetKind RoutedAsset::kind() const noexcept { return assetKindOf(_identity); }

const AssetIdentity& RoutedAsset::identity() const noexcept { return _identity; }

RoutedAssetPhase RoutedAsset::phase() const noexcept { return _phase; }

OptimizerTarget RoutedAsset::target() const noexcept { return _target; }

ExecutionMode RoutedAsset::executionMode() const noexcept { return _executionMode; }

const AssetOperations& RoutedAsset::operations() const noexcept { return _operations; }

SkippedAsset::SkippedAsset(std::filesystem::path executionPath, AssetIdentity identity,
                           const SkipReason reason)
    : _executionPath(std::move(executionPath)), _identity(std::move(identity)), _reason(reason) {}

const std::filesystem::path& SkippedAsset::executionPath() const noexcept { return _executionPath; }

AssetKind SkippedAsset::kind() const noexcept { return assetKindOf(_identity); }

const AssetIdentity& SkippedAsset::identity() const noexcept { return _identity; }

SkipReason SkippedAsset::reason() const noexcept { return _reason; }

RoutingLedger::RoutingLedger(std::vector<RoutedAsset> routedAssets,
                             std::map<SkipReason, std::size_t> skippedAssetCounts) noexcept
    : _routedAssets(std::move(routedAssets)), _skippedAssetCounts(std::move(skippedAssetCounts)) {}

std::span<const RoutedAsset> RoutingLedger::routedAssets() const noexcept { return _routedAssets; }

RoutedAssetReferences RoutingLedger::routedAssets(const RoutedAssetPhase phase) const {
    return matchingRoutedAssets(std::span<const RoutedAsset>(_routedAssets), &RoutedAsset::phase,
                                phase);
}

RoutedAssetReferences RoutingLedger::routedAssets(const OptimizerTarget target) const {
    return matchingRoutedAssets(std::span<const RoutedAsset>(_routedAssets), &RoutedAsset::target,
                                target);
}
...
AssetRouter::AssetRouter(RoutingPolicy policy) noexcept : _policy(std::move(policy)) {}
...
RoutingDecision AssetRouter::route(const std::filesystem::path& executionPath) const {
    ...
    const auto finishDecision = [this, &executionPath, &kindHasWork](
                                    AssetIdentity identity, const RoutedAssetPhase phase,
        ...
        if (!operations.empty()) {
            return RoutedAsset(executionPath, std::move(identity), phase, target,
                               _policy.executionMode(), operations);
        }
        const auto reason =
            kindHasWork(kind) ? SkipReason::ExcludedAssetVariant : SkipReason::DisabledAssetKind;
    ...
    if (extension == ".dds") {
        ...
        return finishDecision(TextureAsset(TextureVariant::Native),
                              RoutedAssetPhase::LooseAssetProcessing, OptimizerTarget::Texture,
                              operations);
    }
    if (extension == ".tga") {
        AssetOperations operations;
    ...
    if (extension == ".nif" || extension == ".btr" || extension == ".bto") {
        ...
        if (_policy.requests(requestedOptimization))
            operations.include(AssetOperation::Optimization);
        // Convertible Texture conversion changes referenced names, so both Mesh Variants carry
        // maintenance independently of optimization.
        if (_policy.maintainsMeshReferences())
            operations.include(AssetOperation::MeshReferenceMaintenance);
        return finishDecision(MeshAsset(variant), RoutedAssetPhase::LooseAssetProcessing,
                              OptimizerTarget::Mesh, operations);
    }
    if (extension == ".hkx") {
        AssetOperations operations;
        if (_policy.requests(RequestedWork::AnimationOptimization))
            operations.include(AssetOperation::Optimization);
        return finishDecision(AnimationAsset{}, RoutedAssetPhase::LooseAssetProcessing,
                              OptimizerTarget::Animation, operations);
    }
    if (extension == _policy.archiveExtension()) {
        ...
        return finishDecision(ArchiveAsset{}, RoutedAssetPhase::ArchiveExtraction,
                              OptimizerTarget::Archive, operations);
    }
...
RoutingLedger AssetRouter::route(
    ...
    for (const auto& executionPath : executionPaths) {
        ...
        // Unsupported paths are intentionally absent because they are neither work nor recognized
        // exclusions.
    }
