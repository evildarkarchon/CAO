...
bool TexturesOptimizer::getDXGIFactory(IDXGIFactory1** pFactory) const {
    ...
    *pFactory = nullptr;
    ...
    if (!sCreateDXGIFactory1) {
        ...
        if (!sCreateDXGIFactory1) return false;
    }
...
bool TexturesOptimizer::createDevice(const int adapter, ID3D11Device** pDevice) const {
    ...
    *pDevice = nullptr;
    ...
    if (!s_DynamicD3D11CreateDevice) {
        ...
        if (!s_DynamicD3D11CreateDevice) return false;
    }
    ...
    if (adapter >= 0) {
        ...
        if (getDXGIFactory(dxgiFactory.GetAddressOf())) {
            if (FAILED(dxgiFactory->EnumAdapters(static_cast<uint>(adapter),
    ...
    HRESULT hr = s_DynamicD3D11CreateDevice(
        pAdapter.Get(), (pAdapter) ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, nullptr,
        createDeviceFlags, featureLevels, _countof(featureLevels), D3D11_SDK_VERSION, pDevice, &fl,
        nullptr);
    if (SUCCEEDED(hr)) {
        if (fl < D3D_FEATURE_LEVEL_11_0) {
            D3D11_FEATURE_DATA_D3D10_X_HARDWARE_OPTIONS hwopts;
            hr = (*pDevice)->CheckFeatureSupport(D3D11_FEATURE_D3D10_X_HARDWARE_OPTIONS, &hwopts,
                                                 sizeof(hwopts));
            if (FAILED(hr)) memset(&hwopts, 0, sizeof(hwopts));
...
TexturesOptimizer::TexOptOptionsResult TexturesOptimizer::processArguments(
    const bool& bNecessary, const bool& bCompress, const bool& bMipmaps,
    ...
    result.bNeedsResize = result.tHeight != _info.height || result.tWidth != _info.width;
...
bool TexturesOptimizer::optimize(const bool& bNecessary, const bool& bCompress,
    ...
    if (options.bNeedsResize) {
        ...
        if (!resize(options.tWidth, options.tHeight)) return false;
    ...
    if (!canBeCompressed()) targetFormat = DXGI_FORMAT_B8G8R8A8_UNORM;

    if (!convert(targetFormat)) return false;
    ...
    switch (type) {
        ...
        case DDS:
            ...
            if (DirectX::IsTypeless(_info.format)) {
                ...
                if (DirectX::IsTypeless(_info.format)) return false;
...
bool TexturesOptimizer::open(const void* pSource, const size_t& size, const TextureType& type,
    ...
    switch (type) {
        case TGA:
            return LoadFromTGAMemory(static_cast<const uint8_t*>(pSource), size, &_info, *_image);
        case DDS:
            const auto ddsFlags = DirectX::DDS_FLAGS_NONE;
            const HRESULT hr = LoadFromDDSMemory(static_cast<const uint8_t*>(pSource), size,
                                                 ddsFlags, &_info, *_image);
            if (FAILED(hr)) return false;
...
bool TexturesOptimizer::decompress() {
    if (!DirectX::IsCompressed(_info.format)) return false;
    ...
    const HRESULT hr =
        Decompress(img, nimg, _info, DXGI_FORMAT_UNKNOWN /* picks good default */, *timage);
    if (FAILED(hr)) {
    ...
    if (!compareInfo(_info, tinfo)) return false;
...
bool TexturesOptimizer::resize(size_t targetWidth, size_t targetHeight) {
    if (_info.width <= targetWidth && _info.height <= targetHeight) return true;
    ...
    const HRESULT hr =
        Resize(imgs, _image->GetImageCount(), _info, targetWidth, targetHeight, filter, *timage);
    if (FAILED(hr)) {
...
bool TexturesOptimizer::generateMipMaps() {
    ...
    if (_info.mipLevels != 1 && _info.mipLevels != tMips) {
        ...
        if (FAILED(hr)) {
            PLOG_ERROR << "Failed to copy texture data to single level (when generating mipmaps) "
                          "when processing: "
                       << _name;
            return false;
        }
        ...
        for (size_t i = 0; i < _info.arraySize; ++i) {
            ...
            if (FAILED(hr)) {
                PLOG_ERROR << "Failed to copy texture data to single level (when generating "
                              "mipmaps) when processing: "
                           << _name;
                return false;
            }
        }
    ...
    if ((_info.width > 1 || _info.height > 1 || _info.depth > 1)) {
        ...
        const auto filter = DirectX::TEX_FILTER_SEPARATE_ALPHA;
        const HRESULT hr = GenerateMipMaps(_image->GetImages(), _image->GetImageCount(),
                                           _image->GetMetadata(), filter, tMips, *timage);
        if (FAILED(hr)) {
...
bool TexturesOptimizer::isCompressed() const { return DirectX::IsCompressed(_info.format); }

DirectX::TexMetadata TexturesOptimizer::getInfo() const { return _info; }

bool TexturesOptimizer::convert(const DXGI_FORMAT& format) {
    if (DirectX::IsCompressed(format)) return convertWithCompression(format);
...
bool TexturesOptimizer::convertWithoutCompression(const DXGI_FORMAT& format) {
    if (_info.format != format) {
        ...
        const HRESULT hr = Convert(
            _image->GetImages(), _image->GetImageCount(), _image->GetMetadata(), format,
            DirectX::TEX_FILTER_FLAGS::TEX_FILTER_DEFAULT, DirectX::TEX_THRESHOLD_DEFAULT, *timage);
        if (FAILED(hr)) {
        ...
        const auto& tinfo = timage->GetMetadata();
        if (tinfo.format != format) return false;
        _info.format = tinfo.format;
...
bool TexturesOptimizer::convertWithCompression(const DXGI_FORMAT& format) {
    if (isCompressed() || _image->GetMetadata().format == format) return true;
    ...
    HRESULT hr;
    if (bc6hbc7 && _pDevice)
        hr = Compress(_pDevice.Get(), img, nimg, _info, format,
                      DirectX::TEX_COMPRESS_FLAGS::TEX_COMPRESS_BC7_USE_3SUBSETS, 1.f, *timage);
    else
        hr = Compress(img, nimg, _info, format,
                      DirectX::TEX_COMPRESS_DEFAULT | DirectX::TEX_FILTER_SEPARATE_ALPHA,
                      DirectX::TEX_THRESHOLD_DEFAULT, *timage);
...
bool TexturesOptimizer::compareInfo(const DirectX::TexMetadata& info1,
    ...
    const bool isSame = info1.width == info2.width || info1.height == info2.height ||
