...
void TexturesOptimizer::listLandscapeTextures(QDirIterator &it) {
  ...
  if (_landscapeTextures.isEmpty()) {
    PLOG_ERROR << "customHeadparts.txt not found. This can cause issue when "
                  "optimizing meshes, as some headparts "
                  "won't be detected.";
  }
...
bool TexturesOptimizer::createDevice(const int adapter,
  ...
  *pDevice = nullptr;
  ...
  HRESULT hr = s_DynamicD3D11CreateDevice(
      pAdapter.Get(),
      (pAdapter) ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, nullptr,
...
bool TexturesOptimizer::optimize(const std::optional<size_t> &tWidth,
  ...
  if (!options.bNeedsCompress && !options.bNeedsResize &&
...
bool TexturesOptimizer::open(const void *pSource, const size_t &size,
                             const TextureType &type, const QString &fileName) {
  PLOG_VERBOSE << "Opening " << fileName << " from memory with textures type "
               << type;
  _name = fileName;
  ...
  modifiedCurrentTexture = false;
  // DirectXTex memory loaders operate on byte buffers, while archive callers
  // pass untyped data.
  const auto bytes = static_cast<const uint8_t *>(pSource);

  switch (type) {
  case TGA:
    return LoadFromTGAMemory(bytes, size, &_info, *_image);
  case DDS:
    const auto ddsFlags = DirectX::DDS_FLAGS_NONE;
    const HRESULT hr =
        LoadFromDDSMemory(bytes, size, ddsFlags, &_info, *_image);
    if (FAILED(hr))
      return false;
...
bool TexturesOptimizer::decompress() {
  ...
  std::unique_ptr<DirectX::ScratchImage> timage(new (std::nothrow)
  ...
  const HRESULT hr = Decompress(
      img, nimg, _info, DXGI_FORMAT_UNKNOWN /* picks good default */, *timage);
  if (FAILED(hr)) {
...
bool TexturesOptimizer::resize(size_t targetWidth, size_t targetHeight) {
  ...
  assert(tinfo.width == targetWidth && tinfo.height == targetHeight &&
         tinfo.mipLevels == 1);
...
bool TexturesOptimizer::generateMipMaps() {
  ...
  if (_info.mipLevels != 1 && _info.mipLevels != tMips) {
    // Mips generation only works on a single base image, so strip off existing
    // mip levels Also required for preserve alpha coverage so that existing
    // mips are regenerated
    std::unique_ptr<DirectX::ScratchImage> timage(new (std::nothrow)
                                                      DirectX::ScratchImage);
    if (!timage) {
      PLOG_ERROR << "Memory allocation failed";
    ...
    if (FAILED(hr)) {
      PLOG_ERROR << "Failed to copy texture data to single level (when "
                    "generating mipmaps) when processing: "
                 << _name;
      return false;
    }
  ...
  if ((_info.width > 1 || _info.height > 1 || _info.depth > 1)) {
    ...
    // Forcing non wic since WIC won't work on my computer, and thus probably on
    // other computers
    const auto filter = DirectX::TEX_FILTER_SEPARATE_ALPHA;
    const HRESULT hr =
        GenerateMipMaps(_image->GetImages(), _image->GetImageCount(),
...
bool TexturesOptimizer::convertWithoutCompression(const DXGI_FORMAT &format) {
  if (_info.format != format) {
    ...
    const HRESULT hr = Convert(_image->GetImages(), _image->GetImageCount(),
...
bool TexturesOptimizer::isIncompatible() const {
  ...
  const bool opaqueAlpha =
      _info.GetAlphaMode() == DirectX::TEX_ALPHA_MODE::TEX_ALPHA_MODE_OPAQUE;
