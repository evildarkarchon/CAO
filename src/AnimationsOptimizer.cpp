...
void AnimationsOptimizer::convert(const QString &filePath) {
  ...
  if (!file.copy(tempHkx)) {
    PLOG_ERROR
        << QString("Cannot copy %1 in order to convert it").arg(filePath);
    return;
  }

  QProcess hkxcmd(this);
  const QString tempHkxFull =
  ...
  const QString output =
      hkxcmd.readAllStandardOutput() + hkxcmd.readAllStandardError();
  const bool success = !output.contains("not loadable");

  if (!success) {
    PLOG_WARNING << QString(
                        "Cannot convert %1, it is probably already converted.")
                        .arg(filePath);
    return;
  }
  ...
  if (!QFile::rename(outHkx, filePath)) {
    PLOG_ERROR << QString(
                      "Failed to convert %1: Cannot copy it back to its path")
                      .arg(filePath);
    return;
  }
