...
AssetWorkPolicy::AssetWorkPolicy(const bool extractArchives,
    ...
    : _extractArchives(extractArchives), _packArchives(packArchives),
...
bool AssetWorkPolicy::allowsArchiveExtractionFor(
  ...
  return _extractArchives &&
         fileName.endsWith(_archiveExtension, Qt::CaseInsensitive);
}
...
bool AssetWorkPolicy::allowsArchivePacking() const { return _packArchives; }
...
bool AssetWorkPolicy::allowsMeshOptimization() const { return _optimizeMeshes; }
