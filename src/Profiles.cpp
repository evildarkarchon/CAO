...
void Profiles::init() {
  _commonSettings =
      new QSettings("profiles/common.ini", QSettings::IniFormat, this);
...
size_t Profiles::findProfiles(const QDir &dir) {
  ...
  for (const auto &subDir :
       _profileDir.entryList(QDir::Dirs | QDir::NoDotAndDotDot)) {
...
void Profiles::loadProfile(const QString &newProfile) {
  ...
  _profileSettings =
      new QSettings(folder + "/profile.ini", QSettings::IniFormat, this);
  _optionsSettings =
      new QSettings(folder + "/settings.ini", QSettings::IniFormat, this);
...
QFile Profiles::getFile(const QString &filename) {
  const QDir &currentProfileDir(getInstance()._profileDir.absoluteFilePath(
      getInstance().currentProfile()));
  const QDir &defaultProfileDir(
      getInstance()._profileDir.absoluteFilePath(defaultProfile));
...
void Profiles::saveToIni() {
  ...
  _profileSettings->setValue("texturesUnwantedFormats",
                             _texturesUnwantedFormats);
  _profileSettings->setValue("texturesCompressInterface",
...
void Profiles::readFromIni() {
  ...
  _maxBsaUncompressedSize =
      _profileSettings->value("maxBsaUncompressedSize").toDouble();
  ...
  _texturesFormat =
      _profileSettings->value("texturesFormat").value<DXGI_FORMAT>();
  _texturesConvertTga = _profileSettings->value("texturesConvertTga").toBool();
  _texturesUnwantedFormats =
      _profileSettings->value("texturesUnwantedFormats").toList();
  _texturesCompressInterface =
      _profileSettings->value("texturesCompressInterface").toBool();
  _profileSettings->endGroup();
}
