...
cao::execution::OperationResult MeshesOptimizer::optimize(
    NifFile& nif, const QString& filepath, const cao::routing::ExecutionMode mode) const {
    ...
    if (mode == cao::routing::ExecutionMode::DryRun) {
        ...
        if (iMeshesOptimizationLevel >= 1 && bMeshesHeadparts &&
            ...
            PLOG_INFO << filepath +
                             " would be optimized as an headpart due to necessary optimization";
            wouldChange = true;
        } else {
            switch (scanResult) {
                case doNotProcess:
...
#undef PRINT
    ...
    return modifiedMesh || processedHeadpart ? cao::execution::OperationResult::changed()
                                             : cao::execution::OperationResult::unchanged();
}

std::tuple<bool, NifFile> MeshesOptimizer::loadMesh(const QString& filepath,
