...
def validate_instance(
    ...
    if reference is not None:
        ...
        if resolved is None:
            return [
                ValidationError(
                    source, pointer or "/", f"unresolved schema reference {reference}"
                )
            ]
    ...
    if "enum" in schema and value not in schema["enum"]:
        errors.append(
            ValidationError(
                source, pointer or "/", f"must be one of {schema['enum']!r}"
            )
        )
    ...
    if isinstance(value, dict):
        ...
        for name, child in value.items():
            ...
            if child_schema is not None:
                errors.extend(
                    validate_instance(
                        child, child_schema, source, child_pointer, root_schema
                    )
                )
            elif schema.get("additionalProperties") is False:
                errors.append(
                    ValidationError(
                        source, child_pointer, "additional property is not allowed"
                    )
                )
    ...
    if isinstance(value, list):
        ...
        if item_schema is not None:
            for index, item in enumerate(value):
                errors.extend(
                    validate_instance(
                        item, item_schema, source, f"{pointer}/{index}", root_schema
                    )
                )
...
def validate_input_semantics(
    ...
    if missing:
        errors.append(
            ValidationError(source, "/inputs", f"missing pinned inputs: {missing}")
        )
    if extra:
        errors.append(
...
def validate_matrix_semantics(
    ...
    if parity_gate and not cells:
        errors.append(
            ValidationError(
                source, "/cells", "parity gate requires declared coverage cells"
            )
        )
    for index, cell in enumerate(cells):
        ...
        if parity_gate and status in {"blocked", "temporary-opt-in", "unknown"}:
            errors.append(
                ValidationError(
                    source, f"{base}/status", f"{status} blocks parity closure"
                )
            )
    return errors
...
def index_versioned_documents(
    ...
    for path, document in documents:
        ...
        if reference in index:
            errors.append(
                ValidationError(
                    path, "/id", f"duplicate versioned identity {reference}"
                )
            )
        else:
...
def validate_cross_references(
    ...
    for path, fixture in fixtures:
        ...
        for engine_index, engine in enumerate(fixture.get("required_engines", [])):
            ...
            if pinned is None:
                ...
                continue
            if engine.get("version") != pinned.get("version") or engine.get(
                "commit"
            ) != pinned.get("commit"):
                errors.append(
                    ValidationError(
        ...
        if reference not in evidence_index:
            ...
            if reference_data.get("bundle_sha256") != evidence_document.get(
                "bundle", {}
            ).get("sha256"):
                errors.append(
                    ValidationError(
        ...
        for index, reference in enumerate(fixture.get("discrepancy_refs", [])):
            ...
            if discrepancy is None:
                ...
                continue
            errors.append(ValidationError(path, f"/discrepancy_refs/{index}", message))
    ...
    for path, evidence_document in evidence:
        ...
        if fixture_record is None:
            errors.append(
                ValidationError(
                    path, "/fixture_ref", f"stale fixture reference {reference}"
                )
            )
        else:
        ...
        if reference_data.get("sha256") != evidence_document.get("environment", {}).get(
            "fixture_sha256"
        ):
            errors.append(
                ValidationError(
    ...
    for cell_index, cell in enumerate(matrix.get("cells", [])):
        resolution = cell.get("resolution", {})
        for reference_index, reference_data in enumerate(
            resolution.get("fixture_refs", [])
        ):
            reference = versioned_reference(
                reference_data.get("id"), reference_data.get("revision")
            )
        ...
        for reference_index, reference in enumerate(
            resolution.get("discrepancy_refs", [])
        ):
            discrepancy = discrepancies.get(reference)
            if discrepancy is None:
                message = f"stale discrepancy reference {reference}"
...
def parse_verification_arguments(values: Sequence[str]) -> dict[str, Path]:
    ...
    for value in values:
        ...
        if not separator or not input_id or not raw_path:
            raise ValueError(
                f"invalid --verify-input {value!r}; expected INPUT_ID=PATH"
            )
...
def verify_acquired_inputs(
    ...
    for input_id, path in requests.items():
        ...
        if method == "tree-sha256":
            if not path.is_dir():
                errors.append(
                    ValidationError(path, "/", "input directory does not exist")
                )
                continue
        ...
        if method == "version-output":
            ...
            try:
                ...
                errors.append(
                    ValidationError(path, "/", f"version command failed: {error}")
                )
                continue
...
def validate_repository(
    ...
    for document_path, schema_path in discover_documents(root):
        ...
        baseline_revision = (
            document.get("baseline_revision") if isinstance(document, dict) else None
        )
    ...
    if input_path in loaded:
        ...
        errors.extend(
            verify_acquired_inputs(
                verification_requests, loaded[input_path], input_path
            )
        )
    ...
    if register_path in loaded:
        errors.extend(
            validate_discrepancy_semantics(loaded[register_path], register_path)
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if errors:
        ...
        print(
            f"baseline validation failed with {len(errors)} error(s)", file=sys.stderr
        )
        return 1
