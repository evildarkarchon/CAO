...
- `baseline/compliance-policy.json` records the GPLv3 combined-work and Slint choice while preserving MPL-2.0 file coverage and notices. It also fixes `/MT` plus Rust `+crt-static` and the Windows 10 22H2 x64 floor. The CRT rule applies to the future Tracetide production graph, not the legacy CMake evidence.
- `build-inputs/` records lock-derived transitive native build closures that become known as the production Cargo graph lands. The Skia source lock is selected by the exact `skia-bindings` lockfile checksum and keeps its network-capable build script on a verified local Git closure.
