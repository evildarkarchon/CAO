...
class VerifyBaselineTests(unittest.TestCase):
    ...
    def write_linked_fixture_and_evidence(self, root: Path) -> tuple[Path, Path]:
        ...
        evidence = {
            ...
            "observations": {
                "raw_inis": [
                    self.write_artifact(
                        evidence_directory, "raw/settings.ini", b"[General]\n"
                    )
                ],
                "html_logs": [
                    self.write_artifact(
                        evidence_directory, "raw/log.html", b"<html></html>\n"
                    )
                ],
                "initial_ui_state": [
                    self.write_artifact(
                        evidence_directory, "raw/initial-ui.json", b"{}\n"
                    )
                ],
                "post_profile_ui_state": [
                    self.write_artifact(
                        evidence_directory, "raw/profile-ui.json", b"{}\n"
                    )
                ],
                "warnings_and_validation": [],
                "progress_and_log_sequence": [
                    self.write_artifact(evidence_directory, "raw/events.json", b"[]\n")
                ],
                "screenshots_and_final_state": [
                    self.write_artifact(
                        evidence_directory, "raw/final.png", b"test-png"
                    )
                ],
                "process_tree_and_helper_invocations": [
                    self.write_artifact(
                        evidence_directory, "raw/processes.json", b"[]\n"
                    )
                ],
                "cancellation_point": None,
            },
            "filesystem": {
                "before_manifest": self.write_artifact(
                    evidence_directory, "normalized/before.json", b"{}\n"
                ),
                "after_manifest": self.write_artifact(
                    evidence_directory, "normalized/after.json", b"{}\n"
                ),
            },
            "normalization_evidence": [
                self.write_artifact(evidence_directory, "raw/filesystem.json", b"{}\n")
            ],
    ...
    def test_unreviewed_schema_revision_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            schema_path = (
                root / "verification/schemas/implementation-inputs.schema.json"
            )
    ...
    def test_pinned_dependency_feature_drift_is_rejected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            slint = next(
                entry for entry in manifest["inputs"] if entry["id"] == "slint"
            )
